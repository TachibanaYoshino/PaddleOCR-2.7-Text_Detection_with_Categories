...
def draw_det_res_and_label(dt_boxes, classes, labels, img, img_name, save_path,colors):
    if len(dt_boxes) > 0:
        import cv2
        index = 0
        src_im = img
...
def main():
    ...
    if "num_classes" in global_config:
        ...
        assert "num_classes is not defined in config file."
        ...
        for file in get_image_file_list(config['Global']['infer_img']):
            ...
            if isinstance(post_result, dict):
                ...
                if "classes" in post_result[0]:
                    draw_det_res_and_label(boxes, post_result[0]["classes"], labels, src_img, file, save_det_path,colors)
                else:
                    draw_det_res(boxes, config, src_img, file, save_det_path)
                # draw_det_res(boxes, config, src_img, file, save_det_path)
