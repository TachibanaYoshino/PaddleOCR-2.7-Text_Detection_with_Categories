...
class Head(nn.Layer):
    def __init__(self, in_channels,num_classes=1, kernel_list=[3, 2, 2], **kwargs):
...
class DBHead(nn.Layer):
    ...
    def __init__(self, in_channels, num_classes=1, k=50, **kwargs):
        ...
        if num_classes != 1:
            self.classes = Head(in_channels, num_classes=num_classes, **kwargs)
        else:
            self.classes = None
...
class PFHeadLocal(DBHead):
    def __init__(self, in_channels,num_classes=1, k=50, mode='small', **kwargs):
        super(PFHeadLocal, self).__init__(in_channels, num_classes, k, **kwargs)
        self.mode = mode
    ...
    def forward(self, x, targets=None):
        ...
        if not self.training:
            ...
            if self.num_classes == 1:
                ...
                classes = paddle.argmax(self.classes(x), axis=1, keepdim=True, dtype='int32')
                return {'maps': 0.5 * (base_maps + cbn_maps), 'cbn_maps': cbn_maps, "classes": classes}
        ...
        if self.num_classes == 1:
            ...
            return {'maps': y,'distance_maps': cbn_maps, 'cbn_maps': binary_maps, "classes": self.classes(x)}